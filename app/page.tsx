...
import { 
  ...
  ArrowUp, ArrowDown, ChevronDown, Check, ZoomIn, ZoomOut, 
  Maximize, Search, X, BarChart3, LogOut, Sun, Moon, Laptop, Loader2
} from 'lucide-react'
...
function YoYBadge({ current, previous }: { current: number, previous: number }) {
    ...
    return (
        <div className={`flex items-center gap-[0.2em] text-[0.7em] font-bold px-[0.5em] rounded-full border shadow-sm ${
            isNeutral ? 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700' : 
            isUp ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800' : 
...
function ControlBox({ label, value, onChange, options, color }: any) {
    return (
        <div className={`bg-white dark:bg-slate-900 px-2 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-1.5 w-full hover:border-${color}-400 dark:hover:border-${color}-500 transition-colors`}>
            <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded shrink-0 transition-colors
              bg-${color}-50 text-${color}-700 
              dark:bg-slate-950 dark:text-${color}-300 dark:border dark:border-${color}-900/50`}>
              {label}
            </span>
            <select value={value} onChange={(e) => onChange(e.target.value)} className="bg-transparent text-xs font-bold text-slate-700 dark:text-slate-200 w-full focus:outline-none cursor-pointer py-1 min-w-12.5 truncate [&>option]:text-slate-800 [&>option]:bg-white">
...
function MultiSelect({ label, options, optionsRaw, selected, onChange }: any) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
       ...
       {isOpen && (
         <div className="absolute top-full left-0 mt-1 w-64 max-h-80 overflow-y-auto bg-white dark:bg-slate-900 border dark:border-slate-700 rounded-lg shadow-xl z-50 p-1 flex flex-col">
            <div className="sticky top-0 bg-white dark:bg-slate-900 z-20 pb-1 border-b border-slate-100 dark:border-slate-800 p-2">
                <Search size={12} className="absolute left-4 top-4 text-slate-400" />
                <input type="text" placeholder="Cari..." className="w-full pl-7 pr-6 py-1.5 text-xs border border-slate-200 dark:border-slate-700 rounded bg-slate-50 dark:bg-slate-800 dark:text-slate-100 focus:outline-none focus:border-blue-500" value={searchTerm} onChange={e=>setSearchTerm(e.target.value)} autoFocus />
                {searchTerm && <button onClick={()=>setSearchTerm("")} className="absolute right-3 top-3.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"><X size={12}/></button>}
            </div>
            <div className="overflow-y-auto max-h-60 pt-1">
                ...
                {filtered.length ? filtered.map((o:any) => {
                    ...
                    return (
                        <div key={o.value||'empty'} onClick={()=>toggle(o.value)} className="flex items-center gap-2 px-3 py-1.5 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 rounded text-xs transition-colors">
                            <div className={`w-3 h-3 rounded border flex items-center justify-center shrink-0 ${isSel?'bg-blue-600 border-blue-600':'border-slate-300 dark:border-slate-600'}`}>{isSel&&<Check size={8} className="text-white"/>}</div>
...
function ThemeToggle() {
    ...
    return (
        <div className="relative" ref={ref}>
           ...
           {isOpen && (
             <div className="absolute top-full right-0 mt-2 w-32 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 overflow-hidden py-1">
                {['light', 'dark', 'system'].map((m: any) => (
                    <button key={m} onClick={() => { setTheme(m); setIsOpen(false); }} className={`flex items-center gap-3 w-full px-3 py-2 text-xs text-left hover:bg-slate-50 dark:hover:bg-slate-700 ${theme === m ? 'text-blue-600 dark:text-blue-400 font-bold' : 'text-slate-600 dark:text-slate-300'}`}>
                        {icons[m]} <span className="capitalize">{m}</span>
                    </button>
                ))}
             </div>
           )}
...
function usePivotLogic({ data, expandedCols, expandedRows, activeLevels }: any) {
  return useMemo(() => {
    ...
    const traverse = (nodes: PivotNode[]) => { 
        nodes.forEach(n => { 
            visibleRows.push(n); 
            if(n.children && expandedRows[n.id]) traverse(n.children) 
        }) 
    }
...
export default function PivotPage() {
  ...
  // ==========================================================================
  // CUSTOM TOOLTIP (COMPACT & BOTTOM TOTAL)
  // ==========================================================================
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        // PERUBAHAN 1: min-w saya naikkan sedikit ke 140px agar layout lebih lega
        <div className="bg-white dark:bg-slate-900 p-2 border border-slate-200 dark:border-slate-800 shadow-xl rounded-lg text-[10px] z-50 min-w-[140px]">
          {/* Header: Tahun */}
            ...
            {payload.slice().reverse().map((entry: any, index: number) => (
              entry.value > 0 && (
                <div key={index} className="flex items-center justify-between gap-3">
                   ...
                   <span className="font-mono text-slate-700 dark:text-slate-300 font-medium text-[10px]">
          ...
          {/* Footer: Total */}
           {/* PERUBAHAN 2: Menambahkan 'gap-4' agar tulisan TOTAL dan Angka tidak mepet */}
           <div className="border-t border-slate-100 dark:border-slate-800 pt-1.5 flex justify-between items-center gap-4 bg-slate-50 dark:bg-slate-800/50 p-1.5 rounded">
              ...
              <span className="font-mono font-bold text-slate-800 dark:text-white text-[10px]">
      ...
      <div className="max-w-400 mx-auto space-y-4">
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col gap-4 z-50 relative transition-colors">
          <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-4">
            <div>
                <h1 className="text-xl font-bold flex items-center gap-2"><LayoutGrid className="text-blue-600 dark:text-blue-500" size={24}/> Sales Analytics</h1>
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 ml-8">Dynamic Pivot & Filters</p>
            </div>
            <div className="flex items-center gap-2">
                <ThemeToggle />
                <div className="h-6 w-px bg-slate-200 dark:bg-slate-700 mx-1"></div>
                <button onClick={() => window.location.reload()} className="p-2 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded hover:bg-blue-100 dark:hover:bg-blue-900/50 border border-blue-100 dark:border-blue-800"><RefreshCcw size={16} className={loading?"animate-spin":""}/></button>
        ...
        {(chartData.length > 0 || loading) && (
            <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm relative z-45 flex flex-col transition-colors min-h-[350px]">
                ...
                {loading && (
                    <div className="absolute inset-0 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm z-50 flex flex-col items-center justify-center rounded-xl transition-all">
                        ...
                        <span className="text-xs font-bold text-slate-500 dark:text-slate-400">Memuat Data...</span>
                    </div>
                )}
                ...
                <div className="h-80 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={trendData} margin={{ top: 10, right: 10, left: 0, bottom: 20 }}>
                            ...
                            <YAxis tickFormatter={yAxisFormatter} tick={{fontSize:10, fill: isDark ? '#94a3b8' : '#64748b'}} axisLine={false} tickLine={false} width={40} />
        ...
        <div className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col items-center gap-3 relative z-40 transition-colors">
           <div className="w-full flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3 mb-1">
              ...
              <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800 px-3 py-1 rounded-full border border-slate-200 dark:border-slate-700">
                  <button onClick={()=>setZoom(p=>Math.max(0.4, Number((p-0.1).toFixed(1))))} className="text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 flex justify-center active:scale-90"><ZoomOut size={14}/></button>
                  <input type="range" min="0.4" max="1.5" step="0.1" value={zoom} onChange={e=>setZoom(parseFloat(e.target.value))} className="w-24 md:w-32 h-1 bg-slate-300 dark:bg-slate-600 rounded-lg appearance-none accent-blue-600"/>
                  <button onClick={()=>setZoom(p=>Math.min(1.5, Number((p+0.1).toFixed(1))))} className="text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 flex justify-center active:scale-90"><ZoomIn size={14}/></button>
                  <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400 w-8 text-right font-bold">{(zoom*100).toFixed(0)}%</span>
              </div>
           </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-300 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col h-[65vh] md:h-[70vh] relative z-0 transition-colors">
          {isRefreshing && <div className="absolute inset-0 z-50 flex items-center justify-center bg-white/70 dark:bg-slate-900/70"><RefreshCcw className="animate-spin text-blue-600" size={24}/></div>}
              ...
              <thead className="bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-200 sticky top-0 z-20 shadow-sm">
                <tr>
                  ...
                  {pivotData.colKeys.map(k => {
                    ...
                    const bgClass = i.type==='year' ? 'bg-slate-100 dark:bg-slate-800' : i.type==='subtotal' ? 'bg-slate-200 dark:bg-slate-700 border-l dark:border-l-slate-600' : 'bg-white dark:bg-slate-900 font-normal text-slate-500 dark:text-slate-500'
                    
                    return <th key={k} className={`p-2 text-center font-bold border-b border-slate-300 dark:border-slate-600 min-w-[6em] ${bgClass}`}>
                        <div className="flex items-center justify-center gap-1">
                           {show && <button onClick={()=>setExpandedCols(p=>({...p,[i.parent]:!p[i.parent]}))} className="hover:text-blue-600 dark:hover:text-blue-400">{expandedCols[i.parent]?<MinusSquare size="1.2em" className="text-red-500 dark:text-red-400"/>:<PlusSquare size="1.2em" className="text-blue-600 dark:text-blue-400"/>}</button>}
                           {i.label}
                        </div>
                    </th>
                  })}
                    ...
                    <tr key={n.id} className="hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors group">
                      <td className="p-2 font-medium text-slate-800 dark:text-slate-200 border-r border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 group-hover:bg-blue-50 dark:group-hover:bg-blue-900/20 sticky left-0 z-10 whitespace-nowrap">
                        <div className="flex items-center gap-[0.5em]" style={{paddingLeft:`${n.level*1.5}em`}}>
                            {!n.isLeaf ? <button onClick={()=>setExpandedRows(p=>({...p,[n.id]:!p[n.id]}))} className="text-slate-400 hover:text-blue-600 dark:hover:text-blue-400">{expandedRows[n.id]?<MinusSquare size="1.2em"/>:<PlusSquare size="1.2em"/>}</button> : <span style={{width:'1.2em'}}/>}
                            <span className={`${n.isLeaf?"text-slate-600 dark:text-slate-400":"font-bold text-slate-800 dark:text-slate-200"} ${n.label.startsWith('No ') ? 'text-red-500 dark:text-red-400 italic' : ''}`}>{n.label}</span>
                        </div>
                      </td>
                      {pivotData.colKeys.map(k => {
                         ...
                         const cellBg = isSub ? 'bg-slate-50 dark:bg-slate-800/50 font-bold border-l border-slate-200 dark:border-slate-700' : ''

                         return <td key={k} className={`p-2 text-right border-r border-slate-100 dark:border-slate-800 align-top ${cellBg}`}>
                            <div className="flex flex-col items-end gap-0.5"><span className={val?'text-slate-900 dark:text-slate-200':'text-slate-300 dark:text-slate-700 font-mono'}>{fmt(val)}</span>{prev>0&&<YoYBadge current={val} previous={prev}/>}</div>
                         </td>
                      })}
                    </tr>
              ...
              <tfoot className="bg-slate-100 dark:bg-slate-800 font-bold text-slate-800 dark:text-slate-200 sticky bottom-0 z-30 shadow-sm">
                <tr>
                    ...
                    {pivotData.colKeys.map(k => {
                        ...
                        return <td key={k} className={`p-3 text-right border-t border-r border-slate-200 dark:border-slate-700 ${i.type==='subtotal'?'bg-slate-200 dark:bg-slate-700':''}`}>
                            <div className="flex flex-col items-end justify-between h-full gap-0.5 min-h-[2.5em]">
                                <span className="font-mono">{fmt(tot)}</span>
                                {prev>0 ? <YoYBadge current={tot} previous={prev}/> : <div className="h-[1em]"></div>}
                            </div>
                        </td>
                    })}
                </tr>
