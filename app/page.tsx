...
const ControlBox = ({ label, value, onChange, options, color }: any) => (
    <div className={`bg-white dark:bg-slate-900 px-2 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-1.5 w-full hover:border-${color}-400 dark:hover:border-${color}-500 transition-colors`}>
        <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded shrink-0 transition-colors bg-${color}-50 text-${color}-700 dark:bg-slate-950 dark:text-${color}-300 dark:border dark:border-${color}-900/50`}>{label}</span>
        <select value={value} onChange={(e) => onChange(e.target.value)} className="bg-transparent text-xs font-bold text-slate-700 dark:text-slate-200 w-full focus:outline-none cursor-pointer py-1 min-w-12.5 truncate [&>option]:text-slate-800 [&>option]:bg-white dark:[&>option]:bg-slate-900 dark:[&>option]:text-slate-200">
...
const MultiSelect = ({ label, options, optionsRaw, selected, onChange }: any) => {
  const [isOpen, setIsOpen] = useState(false), [searchTerm, setSearchTerm] = useState(""), dropdownRef = useRef<HTMLDivElement>(null)
  ...
  useEffect(() => { const clickOut = (e: any) => { if (dropdownRef.current && !dropdownRef.current.contains(e.target)) { setIsOpen(false); setSearchTerm("") } }; document.addEventListener("mousedown", clickOut); return () => document.removeEventListener("mousedown", clickOut); }, [])
    ...
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950 p-2 md:p-6 font-sans text-slate-800 dark:text-slate-100 transition-colors duration-300">
      <div className="max-w-400 mx-auto space-y-4">
        ...
        {(chartData.length > 0 || loading) && (
            <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm relative z-45 flex flex-col transition-colors min-h-87.5">
                {loading && (<div className="absolute inset-0 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm z-50 flex flex-col items-center justify-center rounded-xl transition-all"><Loader2 className="animate-spin text-blue-600 dark:text-blue-400 mb-2" size={32}/><span className="text-xs font-bold text-slate-500 dark:text-slate-400">Memuat Data...</span></div>)}
